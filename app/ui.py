...
class PostSelectionDialog(ctk.CTkToplevel):
    def __init__(self, parent, posts, tr, user_id, service, site, log_callback=None):
        ...
        for post in posts:
            ...
            total_size = metrics.get("total_video_size") or 0
            if total_size:
                display_text += f"\n{tr('Total video size')}: {self._size_formatter(total_size)}"
            ...
            if detail:
                ...
                if size_bytes is None:
                    self._log_debug(
                        ...
                        f"size={size_bytes}, keys={list(entry.keys()) if isinstance(entry, dict) else type(entry)}"
                    )
            else:
                self._log_debug(
